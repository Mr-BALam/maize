...
def register_user(email, password):
    ...
    print(f"Code to be sent are: {code}")
    if db_connected:
        try:
...
if not st.session_state.authenticated:
    ...
    if menu == "Register":
        ...
        if st.button("Register"):
            state, user_code = register_user(email, password)
            if state:
                st.success(f"Check your email for confirmation code.{user_code}")
    ...
    if st.session_state.page == "Dashboard":
        ...
        with col2:
            years = list(range(2018, datetime.now().year + 10))
            selected_year = st.selectbox("Select year", years, index=len(years)-5)
